...
//int main() {
//
//	cv::Mat top_left, top_right, bottom_right;
//	top_left = cv::imread("top_left.jpg");
//	top_right = cv::imread("top_right.jpg");
//	bottom_right = cv::imread("bottom_right.jpg");
//
//	top_left.convertTo(top_left, CV_8U);
//	top_right.convertTo(top_right, CV_8U);
//	bottom_right.convertTo(bottom_right, CV_8U);
//
//	vector<Mat> mTemplates;
//	mTemplates.push_back(top_left);
//	mTemplates.push_back(top_right);
//	mTemplates.push_back(bottom_right);
//
//	VideoCapture capture(0);
//	Mat image;
//
...
//			cv::Mat resultImage;
//
//		for(std::vector<cv::Mat>::iterator temp = mTemplates.begin(); temp != mTemplates.end(); ++temp) {
//			cv::Mat currTemp;
//			temp->copyTo(currTemp);
//
//			cv::matchTemplate(image, top_left, resultImage, cv::TM_SQDIFF);
...
//
//			cv::minMaxLoc(resultImage, &minVal, &maxVal, &minLoc, &maxLoc, cv::Mat());
//			if (maxVal >= 30 && matchLoc.x != 0 && matchLoc.y != 0)
//			{
//				rectangle(traces, Point(matchLoc), Point(matchLoc.x + currTemp.cols, matchLoc.y + currTemp.rows), Scalar(0,0,255));
//			}
